...
class ModelExecutor(hass.Hass):
    def initialize(self):
        ...
        self.states_db = "/ecologichome_src/appdaemon/apps/tsh.db"
        self.last_states = self.get_state()
        self.last_event_time = datetime.datetime.now()
        self.init_db()
        self.log("Hello from ecologichome")
        self.log("ecologichome Model Executor fully initialized!")

    def read_actuators(self):
        enabled_actuators = set()
        with open(
            "/ecologichome_src/frontend/static/data/metrics_matrix.json", "r"
        ) as f:
