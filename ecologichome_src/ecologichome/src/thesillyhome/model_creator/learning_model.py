...
import ecologichome.model_creator.read_config_json as tsh_config
...
def save_visual_tree(model, actuator, feature_vector):
    ...
    plt.savefig(f"/ecologichome_src/frontend/static/data/{actuator}_tree.png")
    plt.close()
...
def train_all_actuator_models():
    ...
    df_metrics_matrix.to_pickle(f"/ecologichome_src/data/model/metrics.pkl")
    ...
    best_metrics_matrix.to_json(
        "/ecologichome_src/frontend/static/data/metrics_matrix.json", orient="records"
    )
...
def train_all_classifiers(
    ...
    plt.savefig(
        f"/ecologichome_src/frontend/static/data/{actuator}_precision_recall.png"
    )
    plt.close()
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        filename="/ecologichome_src/log/ecologichome.log",
        encoding="utf-8",
        level=logging.INFO,
        format=FORMAT,
    )
